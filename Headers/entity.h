...
public:
  ...
  virtual void setAtkBonus(int bonus) { m_atkBonus = bonus < -m_atk ? -m_atk : bonus; }
  virtual void setDefBonus(int bonus) { m_defBonus = bonus < -m_def ? -m_def : bonus; }
...
private:
  int m_hp, m_atk, m_def, m_atkBonus, m_defBonus, m_value, m_chamber, m_maxHp, m_gold;
  bool m_hostile;
  Utility::Type m_type;
};
